...
public class TunerServiceImpl extends TunerService {
    ...
    private boolean isBenzoSetting(String key) {
        return isBenzoGlobal(key) || isBenzoSystem(key) || isBenzoSecure(key);
    }

    private boolean isBenzoGlobal(String key) {
        return key.startsWith("benzoglobal:");
    }

    private boolean isBenzoSystem(String key) {
        return key.startsWith("benzosystem:");
    }

    private boolean isBenzoSecure(String key) {
        return key.startsWith("benzosecure:");
    }
    ...
    private String chomp(String key) {
        return key.replaceFirst("^(benzoglobal|benzosecure|benzosystem|system):", "");
    }

    @Override
    public String getValue(String setting) {
        if (isBenzoGlobal(setting)) {
            return Settings.Global.getString(mContentResolver, chomp(setting));
        } else if (isBenzoSecure(setting)) {
            return Settings.Secure.getStringForUser(
                    mContentResolver, chomp(setting), mCurrentUser);
        } else if (isBenzoSystem(setting)) {
    ...
    private void addTunable(Tunable tunable, String key) {
        ...
        if (!mListeningUris.containsKey(uri)) {
            ...
            mContentResolver.registerContentObserver(uri, false, mObserver,
                    isBenzoGlobal(key) ? UserHandle.USER_ALL : mCurrentUser);
        }
    ...
    public void clearAllFromUser(int user) {
        ...
        for (String key : mTunableLookup.keySet()) {
            if (ArrayUtils.contains(RESET_EXCEPTION_LIST, key) || isBenzoSetting(key)) {
                continue;
            }
            setValue(key, null);
        }
    ...
    private class Observer extends ContentObserver {
        ...
        public void onChange(boolean selfChange, java.util.Collection<Uri> uris,
            ...
            for (Uri u : uris) {
                ...
                if (userId == mUserTracker.getUserId() || isBenzoGlobal(key)) {
                    reloadSetting(u);
                }
            }
        }
