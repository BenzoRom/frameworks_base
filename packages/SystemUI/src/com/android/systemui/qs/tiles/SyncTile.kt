...
class SyncTile @Inject constructor(
    ...
    override fun composeChangeAnnouncement(): String {
        return when {
            state.value -> {
                with(mContext) { getString(R.string.accessibility_quick_settings_sync_changed_on) }
            }
            else        -> {
                with(mContext) { getString(R.string.accessibility_quick_settings_sync_changed_off) }
            }
        }
    }
}
