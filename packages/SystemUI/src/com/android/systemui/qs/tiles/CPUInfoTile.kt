...
class CPUInfoTile @Inject constructor(
    ...
    override fun getTileLabel(): CharSequence {
        return mContext.getText(R.string.quick_settings_cpuinfo_label)
    }

    override fun handleSetListening(listening: Boolean) {
        super.handleSetListening(listening)
        with(globalSetting) { setListening(listening) }
    }

    override fun handleDestroy() {
        super.handleDestroy()
        with(globalSetting) { setListening(false) }
    }

    override fun handleClick(view: View?) {
        val value = if (globalSetting.value != 1) 1 else 0
        with(globalSetting) { setValue(value) }
    }

    override fun handleUpdateState(state: BooleanState, arg: Any?) {
        val enable = (arg as? Int ?: globalSetting.value) != 0
        state.value = enable
    ...
    override fun composeChangeAnnouncement(): String {
        return when {
            state.value -> with(mContext) {
                getString(R.string.accessibility_quick_settings_cpuinfo_on)
            }
            else        -> with(mContext) {
                getString(R.string.accessibility_quick_settings_cpuinfo_off)
            }
        }
    }
